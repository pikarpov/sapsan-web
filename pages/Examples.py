...
with st.sidebar.expander('Model'):
    st.number_input(label='# of Epochs',value=config['n_epochs'],key='n_epochs',min_value=1,max_value=100,format='%d')
...
with st.expander("Show model code"):            
    ...
    if st.button('Edit'):
        st.info('***Code editing is only available in the [local GUI version](https://sapsan-wiki.github.io/overview/examples/local_examples/)***',
                icon='❗')
...
if st.session_state.backend_selection == 'MLflow':        
    st.info('***[MLflow](https://mlflow.org/) interface is only available in the [local GUI version](https://sapsan-wiki.github.io/overview/examples/local_examples/)***  \n  '\
            'Usage in Sapsan: [sapsan-wiki.github.io/tutorials/mlflow](https://sapsan-wiki.github.io/tutorials/mlflow/)', icon='❗')
